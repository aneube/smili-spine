/*=========================================================================
  The Software is copyright (c) Commonwealth Scientific and Industrial Research Organisation (CSIRO)
  ABN 41 687 119 230.
  All rights reserved.

  Licensed under the CSIRO BSD 3-Clause License
  You may not use this file except in compliance with the License.
  You may obtain a copy of the License in the file LICENSE.md or at

  https://stash.csiro.au/projects/SMILI/repos/smili/browse/license.txt

  Unless required by applicable law or agreed to in writing, software
  distributed under the License is distributed on an "AS IS" BASIS,
  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
  See the License for the specific language governing permissions and
  limitations under the License.
=========================================================================*/
#ifndef milxQTSPINEMODEL_H
#define milxQTSPINEMODEL_H

#include "milxQtModel.h"

class MILXQT_PLUGIN_EXPORT milxQtSpineModel : public milxQtModel
{
    Q_OBJECT

public:
    milxQtSpineModel(QWidget *theParent = 0);
    virtual ~milxQtSpineModel();

public slots:
//    void colourByDirection();
//    void harmonics(QString ordersString = "");

protected:
	//QAction *colourDirectionAct; //!< colour by direction axes action
    //QAction *harmonicsAct; //!< Display spherical harmonic

private:
    void createActions();
    void createConnections();
    void contextMenuEvent(QContextMenuEvent *currentEvent);

};

#endif // milxQtSpineMODEL_H
